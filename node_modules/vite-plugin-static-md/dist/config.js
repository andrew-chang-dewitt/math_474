...
import { logger as getLogger, replace as replaceLogger, } from "./logging.js";
...
export async function modifyConfig(userConfig, opts, mode) {
...
async function expandExcludes(list, mode) {
    let res = [];
    if (!list)
        return res;
    if (isString(list)) {
        ...
        logger.warn("excludes must be of type `string | string[] | { serve: string | string[], build: string | string[] }`\n" +
            `ignoring given value of ${dir(list)}`);
    }
    return (await Promise.all(res.map(async (path) => {
        let res = [];
...
function isArrayOf(obj, checkT) {
    return Array.isArray(obj) && obj.every(checkT);
}
function isExcludePatterns(obj) {
    const serveCorrect = "serve" in obj
...
function buildInputObj(entries, root) {
