...
export async function getPaths(root, exclude) {
 ...
 */
export function getURL(path, root) {
    const out = getOutputRelativePath(path, root);
    return out.length === 0 ? "/" : `/${out}/`;
}
export function getHtmlId({ dir, name }) {
    let res = `${dir}/${name}`;
 ...
 */
export function getOutputRelativePath({ dir, name }, root) {
    let res = "";
 ...
 */
export function getRollupInputKey(path, root) {
    const out = getOutputRelativePath(path, root);
    return out.length === 0 ? "main" : out;
}
export function getInputRelativePath({ dir, base }, root) {
    let res = "";
