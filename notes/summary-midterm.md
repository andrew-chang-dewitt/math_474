...
- **partitioning n objects into r cells** ($n_1, \ldots, n_r$ per cell, $\sum n_i = n$): $\binom{n}{n_1,\ldots,n_r} = n!/(n_1!\cdots n_r!)$
...
- **covariance** $\sigma_{XY} = E[(X-\mu_X)(Y-\mu_Y)]$; 0 if `X`,`Y` independent (converse not generally true &mdash; only captures linear relation)
...
- **variance of linear combo (2 RVs)** $\sigma^2_{aX+bY+c} = a^2\sigma_X^2 + b^2\sigma_Y^2 + 2ab\sigma_{XY}$
  - const doesn't change var; scaling by `a` scales var by $a^2$; if independent, cross term drops (holds for $\pm$); for `n` mutually independent $X_i$: $\sigma^2_{\sum a_iX_i} = \sum a_i^2\sigma_{X_i}^2$
- **Chebyshev's theorem**: $P(\mu-k\sigma < X < \mu+k\sigma) \ge 1-1/k^2$; distribution-free, lower bound only, use when distribution form unknown
...
- **Poisson distribution** $p(x;\lambda t) = e^{-\lambda t}(\lambda t)^x/x!$, `λ` = avg outcomes per unit time/space, $\mu=\lambda t$; mean=var=$\lambda t$
...
- **standard normal** ($\mu=0,\sigma^2=1$): $Z = (X-\mu)/\sigma$; $P(x_1<X<x_2)=P(z_1<Z<z_2)$
...
- **sum of independent chi-squareds**: $Y=X_1+\cdots+X_n$ (df $v_1,\ldots,v_n$) is chi-squared w/ $v=\sum v_i$ (also reproductive)
