...
> $$
>
> Properties of the normal curve: mode at $x = \mu$; symmetric about
> $x = \mu$; inflection points at $x = \mu \pm \sigma$; approaches the
> horizontal axis asymptotically; total area under the curve is 1.

### theorem 6.2, mean & variance of the normal distribution

> [!IMPORTANT]
>
> The mean and variance of $n(x; \mu, \sigma)$ are `μ` & $\sigma^2$,
...
> [!IMPORTANT]
>
> If `X` is a binomial random variable with mean $\mu = np$ & variance
> $\sigma^2 = npq$, then the limiting form of the distribution of
>
> $$
...
> $$
>
> Properties: (a) $\Gamma(\alpha) = (\alpha - 1)\Gamma(\alpha - 1)$
> (recursion, for $\alpha > 1$); (b) $\Gamma(n) = (n-1)!$ for positive
> integer `n`; (c) $\Gamma(1) = 1$; (d) $\Gamma(1/2) = \sqrt{\pi}$.
...
> event, has $P(X > x) = e^{-\lambda x}$, giving density
> $f(x) = \lambda e^{-\lambda x}$ with $\lambda = 1/\beta$.
...
> $$
>
> The uniform distribution on $(0,1)$ is the special case
> $\alpha = \beta = 1$.
...
> [!IMPORTANT]
>
> The continuous random variable `X` has a **lognormal distribution** if
> $Y = \ln(X)$ has a normal distribution with mean `μ` & standard
